...
class Mage_Core_Model_Layout_Update
{
	...
	public function asSimplexml()
	{
		...
		return SimpleXml_Dom_Element_Adapter::loadFromString($updates, $this->getElementClass());
	}
...
//        }
	...
	public function fetchFileLayoutUpdates()
	{
		...
		if (Mage::app()->useCache('layout') && ($layoutStr = Mage::app()->loadCache($cacheKey))) {
			...
			$this->_packageLayout = SimpleXml_Dom_Element_Adapter::loadFromString($layoutStr, $elementClass);
		}
		if (empty($layoutStr)) {
...
#echo '<textarea style="width:600px; height:400px;">'.$handle.':'.print_r($updateXml,1).'</textarea>';
	...
	public function fetchDbLayoutUpdates($handle)
	{
		...
		$updateXml = SimpleXml_Dom_Element_Adapter::loadFromString($updateStr, $this->getElementClass());
		$this->fetchRecursiveUpdates($updateXml);
		$this->addUpdate($updateXml->innerXml());
	...
	public function getFileLayoutUpdatesXml($area, $package, $theme, $storeId = null)
	{
		...
		foreach ($updateFiles as $file) {
			...
			$fileStr = str_replace($this->_subst['from'], $this->_subst['to'], $fileStr);
			//$fileXml = simplexml_load_string($fileStr, $elementClass);
			$fileXml = SimpleXml_Dom_Element_Adapter::loadFromString($fileStr, $elementClass);
			if (!$fileXml instanceof SimpleXMLElement) {
				continue;
			}
			$layoutStr .= $fileXml->innerXml();
		...
		//$layoutXml = simplexml_load_string('<layouts>'.$layoutStr.'</layouts>', $elementClass);
		$layoutXml = SimpleXml_Dom_Element_Adapter::loadFromString('<layouts>'.$layoutStr.'</layouts>', $elementClass);
		return $layoutXml;
	}
}
